  ...
  return (
    <div className={`popup ${isOpen ? "popup_opened" :''}`} id={`popup_${name}`} >
      <div className={`popup__container popup__container-${name}`}>
        ...
        <form className="popup__inform" name={name} onSubmit={onSubmit} noValidate>
        {children}
        <button className="popup__button-rectangle" aria-label="Save" type="submit" name="save" id="save">{button}</button>
        </form>
      </div>
    </div>
    )
